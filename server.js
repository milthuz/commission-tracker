...
app.get('/api/auth/zoho', (req, res) => {
  ...
  const authUrl = `${ZOHO_CONFIG.accounts_url}/oauth/v2/auth?` +
    `scope=ZohoBooks.invoices.READ,ZohoBooks.invoices.CREATE,ZohoBooks.invoices.UPDATE,ZohoSubscriptions.plans.READ,ZohoSubscriptions.products.READ,AaaServer.profile.READ` +
...
app.get('/api/invoices/enrich-preview/:invoiceNumber', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // For SaaS invoices generated by a recurring invoice template, the template
    // has start_date. We fetch the recurring invoice to extract it.
    ...
    if (!subActivation && inv.recurring_invoice_id) {
      try {
        ...
        if (rec) {
          subActivation = rec.start_date || rec.cf_subscription_activation_date || null;
          recurringInvoiceDebug = {
            recurring_invoice_id: rec.recurring_invoice_id,
            start_date: rec.start_date,
            status:     rec.status,
            recurrence_frequency: rec.recurrence_frequency,
    ...
    if (invoiceType === 'hardware' && inv.customer_id) {
      ...
      for (const ci of customerInvoices) {
        ...
        if (hasSaas) {
          ...
          // Fall back to the recurring invoice template's start_date
          if (!activationDate && det_inv.recurring_invoice_id) {
            try {
              const recRes = await axios.get(
                `${admin.api_domain}/books/v3/recurringinvoices/${det_inv.recurring_invoice_id}`,
                {
                  params: { organization_id: process.env.ZOHO_ORG_ID },
                  headers: { Authorization: `Zoho-oauthtoken ${accessToken}` },
                  validateStatus: () => true,
              ...
              activationDate = recRes.data?.recurring_invoice?.start_date || null;
            } catch (_e) { /* keep null */ }
          }
